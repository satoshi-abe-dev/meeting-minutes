...
重大な懸念（機密混入・破壊的操作）ではない通常の指摘。`SESSION_RULES.md`
「manager によるPRレビュー・マージ」の分岐に従い、**本人への報告はこの往復の途中では
行わず、マージ後にまとめて行う**（手順7〜8、新機能フローなら `new-feature` の最終報告）。
個々の指摘ごとに本人へ即時報告する必要はない。

`SendMessage` で worker に具体的な指摘内容（該当箇所・再現条件・対処方針）を送り、
マージを保留する。修正コミットが来たら、**手順1から（`git fetch origin <ブランチ名>`
での再フェッチと機密チェックを含めて）1〜4を再度実行する**。この往復は何度でも
繰り返す。理由:

- 再フェッチしないと `FETCH_HEAD` が古いままで、修正版でなく旧コミットを
  レビュー・テストしてしまう。
- 修正コミットで**新たに機密が混入する**こともある。毎回手順1の機密チェックを
  通すことで、混入時は手順1側の分岐でこの「それ以外」ループを抜けて上の
  「機密混入を検出した場合」へ入る。「2〜4だけ」の再実行だと機密チェックを
  飛ばして手順6のマージに進んでしまう。
