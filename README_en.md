...
- **Map-reduce for long transcripts** — a one-hour meeting does not fit in the context window, so it switches to a two-stage chunk-summarize → merge. Chunk summaries are written incrementally to `minutes_partials.json`, so a re-run after a timeout or interruption does not redo the finished parts.
- **Per-stage elapsed time shown in the log** — audio extraction, transcription, frame extraction, frame analysis, each chunk summary, and the minutes merge all append "elapsed X" to their completion messages.
...
- **loop engineering (not a one-shot review, but a verification loop repeated until it converges) was put into practice** — implement → independent verification (pytest, confidential-data grep, diff review, Codex review) → if there is a finding, send it back with the exact location, reproduction conditions, and a fix approach → fix → re-verify, repeated until every check is clear. In Auto mode (Issue #34, PRs #19 / #35) more than six token-budget bugs were fixed over these round trips. The problem where the settings frame was invisible on a real screen was solved over the four-stage round trip of PRs #33 → #40 → #41 → #42
- **when a class of finding recurs, the operational rules themselves are updated, not just the individual PRs** — the same finding, "a file move drops the old-path guard entry in `.gitignore`", occurred three times in a row across PRs #48 / #60 / #66, so "always keep the old-path ignore entry" was written down as a rule from then on (a structure where the loop improves the loop itself)
...
| Misrecognition of song / BGM sections | Hallucinations that repeat the same phrase endlessly are handled with `condition_on_previous_text=False`. However, one-off mishearings (a single odd line) are a known trait of Whisper-family models in general and can remain. For meetings with a lot of singing or applause, eyeballing the transcript is recommended |
| No speaker diarization | There is no "who spoke" label (everything is written as `参加者` / "participant") |
| Reasoning models are ill-suited to minutes generation | Reasoning (thinking) models such as Qwen3 / DeepSeek-R1 spend a large amount of tokens and time on invisible "thinking" and are extremely slow / return empty bodies. **Use a non-reasoning Instruct model.** An empty response is surfaced as an error saying "used up max_tokens on thinking" |
| Resume from mid-way | Transcription, frame extraction, **frame analysis (per frame)**, and **chunk summaries of the minutes (per chunk)** all reuse existing intermediate files and **only re-run what was redone** (disable with `--fresh` / the GUI checkbox). Before processing starts, connectivity to the LLM server and the model is checked so it **fails early** |
| Interruption is not fully immediate | The GUI "Stop" reacts at boundaries: per stage, per frame in frame analysis, per chunk in the minutes. However, **during an mlx-whisper transcription call** and **during ffmpeg execution** it cannot be cut off mid-way (it waits for that stage to finish) |
