...
> - **Decoupling the minutes' language from the recording's language** — motivated by
>   a concrete case: "record an English meeting on a business trip, then write the
>   minutes in Japanese for internal reporting back home." Directed the minutes'
>   output language to be configurable independently of the transcription and GUI
>   display languages, and had frame-capture descriptions (VLM) extracted in the
>   recording's own language rather than the target one, to avoid quality loss
...
- Separately, start a local LLM server. E.g. **LM Studio** (Settings → Local Models → Local Model API: turn on "Local API server"; "Just-in-time model loading" is also recommended). Other OpenAI-compatible servers (Ollama, etc.) work too → [`docs/setup_en.md`](docs/setup_en.md)
...
- GUI display language priority: default `"en"` < `config.toml`'s `[gui] language`
  ...
  - Example of setting it via the environment variable:
    - bash/zsh: `MM_GUI_LANGUAGE=ja python src/meeting_minutes/gui.py`
    - Windows cmd.exe: `set "MM_GUI_LANGUAGE=ja" && python ...`
    - PowerShell: `$env:MM_GUI_LANGUAGE="ja"; python ...`
