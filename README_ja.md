...
会議の音声や資料には顧客情報や未公開の経営・人事情報が含まれることが多く、
NDA・個人情報保護法・GDPR・社内規程などの制約から、録音データをクラウドの
文字起こし・要約サービスへ送れない現場がある。議事録の作成は AI による自動化に
向く一方で、こうした現場ではクラウド型サービスを使いにくい。
...
> 💡 **動かすだけなら、ここまで読めば十分。** この先は、パイプラインの内部設計
> （`pipeline.run` / `Deps` による分割）と AI 協調開発の運用フローの解説。
...
- 例: おまかせモード（Issue #34、PR #19 / #35）では、トークン予算まわりの不具合が 6 件以上
  見つかり、往復のなかで修正した
- 例: GUI の設定枠が実機で見えない問題は、PR #33 → #40 → #41 → #42 と 4 回のやり直しを経て直した
