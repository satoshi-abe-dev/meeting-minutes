...
**打ち合わせの録画動画から議事録（Markdown）を作成する。処理はすべて手元の PC で完結し、音声も文字起こしも要約も外部に送信しない。**
...
> - **worker/manager 2セッションの開発統治** — 実装担当とレビュー・マージ権限担当を
>   分離し、別ベンダー（Codex）の独立レビューを必須にした
> - **「おまかせ」モードの発案** — 動画内容に応じてLLMに見出し構成を提案させ、
>   気に入ればテンプレート化できる仕組み
> - **GUI 全体の設計**
...
議事録作成を AI で自動化したい。しかし会議の音声・資料には顧客情報や未公開の経営・
人事情報が含まれることが多く、NDA・個人情報保護法・GDPR・社内規程などの制約で、
録音データをクラウドの文字起こし・要約サービスへ送れない現場がある。
...
- 生成された構成は `{title}` 等をプレースホルダーのまま残した形で、出力フォルダーの
  **`structure_used.txt`** に保存される
- 生成に失敗した場合（LLM エラー・プレースホルダー欠落・構成が大きすぎて統合に
  収まらない等）は **内蔵にフォールバックし警告**する（`structure_used.txt` は残さない）

**気に入った構成を固定テンプレートにする**（毎回 LLM に生成させない分だけ速く、結果もぶれない）:

```sh
cp output/<動画名>/structure_used.txt templates/<客先名>.txt
```
...
| 歌・BGM区間の誤認識 | 同一フレーズの無限リピートは `condition_on_previous_text=False` で対策済み。単発の空耳は Whisper 系全般の癖で残りうるので、歌・拍手が多い会議は文字起こしの目視確認を推奨 |
| 話者分離なし | 「誰が話したか」のラベルは付かない（`参加者` 表記） |
| 推論モデルは議事録生成に不向き | Qwen3 / DeepSeek-R1 等の推論モデルは見えない「思考」に時間とトークンを大量消費し、極端に遅い／本文が空で返る。**非推論の Instruct 系を使うこと**（空応答は「思考で max_tokens を使い切った」というエラーで気づける） |
| 途中から再開 | 文字起こし・フレーム抽出・**フレーム解析（1枚単位）**・**チャンク要約（1つ単位）**で中間ファイルを再利用し、やり直した分だけ再実行（`--fresh` / GUI のチェックで無効化）。開始前に LLM サーバーとの疎通を確認して早めに失敗する |
| 中断は完全には即時でない | GUI の「中断」はステージ／フレーム／チャンクの節目で反応する。ただし **mlx-whisper の文字起こし中** と **ffmpeg 実行中** はその工程が終わるまで待つ |
| GUI から設定変更不可 | `config.toml` を直接編集する |

LLM がタイムアウト・切断したときの自動リトライは未対応（今のところは再実行で復帰する）。
...
- **長い文字起こしの map-reduce** — 1 時間の会議はコンテキストに収まらないので、
  チャンク要約 → 統合の 2 段に切り替える。要約は `minutes_partials.json` に逐次保存し、
  再実行時は終わった分をやり直さない。
...
- 例: おまかせモード（Issue #34、PR #19 / #35）でトークン予算まわりのバグを 6 件以上、
  往復で潰した
- 例: GUI の設定枠が実機で見えない問題を PR #33 → #40 → #41 → #42 の 4 段階で解決した
- **指摘パターンが繰り返されたら運用ルール自体を更新する** — 個々の PR を直すだけでなく、
  ループがループ自体を改善する構造にしている（例: `.gitignore` の旧パス保護漏れが
  PR #48・#60・#66 で 3 回連続 → 以後「旧パスの ignore エントリを必ず残す」を方針として明文化）
