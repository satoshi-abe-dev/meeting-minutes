...
- **異なるベンダーのモデルによる独立レビューも組み込んだ** — Claude（manager）の判断に加え、
  ...
  追加し、実際に運用している
- **ループエンジニアリング（単発レビューではなく、収束するまで繰り返す検証ループ）を実践した** — 実装 → 独立検証
  （pytest・機密 grep・diff 確認・Codex レビュー）→ 指摘があれば該当箇所・再現条件・対処方針を
  添えて差し戻し → 修正 → 再検証、を全チェックがクリアになるまで繰り返す。おまかせモード
  （Issue #34、PR #19 / #35）ではトークン予算まわりのバグを 6 件以上、往復で潰した。GUI の
  設定枠が実機で見えない問題は PR #33 → #40 → #41 → #42 の 4 段階の修正往復で解決した
- **指摘のパターンが繰り返されたら、個々の PR を直すだけでなく運用ルール自体を更新する** —
  「ファイル移動時に `.gitignore` の旧パス保護エントリを消してしまう」という同種の指摘が
  PR #48・#60・#66 で 3 回連続したため、以後は「旧パスの ignore エントリを必ず残す」を方針として
  明文化した（ループがループ自体を改善する構造）
