...
- **議事録の言語を独立して設定可能に** — `[output] minutes_language` を追加。
  `[transcribe] language`（会議音声の言語）・`[gui] language`（画面文言）とは独立に
  議事録の出力言語を指定できる（既定 `"ja"`、従来どおり）。画面キャプチャの説明文
  （VLM）はこの設定に連動させず、Whisper の検出結果（`transcribe_wav` の戻り値）から
  録画の実際の言語を自動的に採用する — 目的言語への翻訳は議事録生成時の一度きりに
  留め、二重翻訳による劣化を避けるため。プロンプトファイルを言語ごとに増やす代わりに、
  システムプロンプト中の `{lang}` プレースホルダー置換／末尾への指示文追記で対応
