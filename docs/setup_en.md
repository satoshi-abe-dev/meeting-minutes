...
| The minutes come out in an unexpected language | Set `config.toml`'s `[output] minutes_language` explicitly (default `"ja"`). `[transcribe] language` is the recorded meeting audio's language — separate from the minutes' language. The frame-analysis (VLM) description language is derived automatically from the recording's own detected language (no setting needed). E.g. for an English meeting recorded on a business trip, with Japanese minutes wanted for reporting back home, keep `[transcribe] language = "en"` and set `[output] minutes_language = "ja"`. Also use an LLM that handles the target language well. |
